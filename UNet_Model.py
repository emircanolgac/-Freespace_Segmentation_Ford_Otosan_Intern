...
#Create a function with 2 parameters named double_conv
    if not mid_channels:
        mid_channels = out_channels
        return nn.Sequential(
        #nn.Sequential allows you to create a neural network by specifying the 
        #building blocks (nn.Modules) of that network sequentially.
        
            nn.Conv2d(in_channels, mid_channels, kernel_size=3,padding=1),
        
        # conv2D is a function of convolution to a 2D data (eg an image).
        # Convolution is a mathematical operator that produces a 
        # result when a data string goes through a specific process.
        # Think of convolution as applying a filter to our image.
        # The Conv2D function can perform convolution 
        # to a much more complex data structure 
        # (not just a single 2D data but a 2D data set) and some additional 
        # options like padding and stride.
        ...
        # padding (int or tuple, optional) - Zero padding added on both sides 
        # of the entry (Default: 0)
        # in_channels 3 channels (color images) is initially 3 for images. 
        # It should be 1 for black and white images. 
        # Some satellite images should have a 4.
        
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),#activation function
        #The Non-Linearity layer is usually followed by all the Convolutional layers.
        #This layer is called the activation layer (Activation Layer) 
        #because it uses one of the activation functions.
        #A neural network without the activation function will act as a 
        #linear regression with limited learning power
        ...
        #The ReLU function takes the value 0 for negative inputs, while x takes the 
        #x value for positive inputs.
        #inplace = True means it will directly change the input without 
        #allocating any additional output.
            nn.Conv2d(mid_channels, out_channels, kernel_size=3,padding=1),
            nn.BatchNorm2d(out_channels),
        #Normalizes the input layer by re-centering and rescaling neural networks, 
        #a method used to make it faster and more stable
